...
public class TransactionsService : ITransactionsService
{
    ...
    public async Task<(List<Transaction> Items, long Total)> GetPagedAsync(int skip, int limit)
    {
        long total = await _transactionsCollection.CountDocumentsAsync(FilterDefinition<Transaction>.Empty);
        List<Transaction> items = await _transactionsCollection
            .Find(_ => true)
